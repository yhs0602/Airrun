...
HBITMAP hbititem[ITEMKINDS];
HBITMAP hbitsurround[5];
HBITMAP hbitenemy;
HBITMAP hbitbullet[BULLETLEVELS],hbitnuclear;
...
//            DeleteObject(hbitnuclear);
        ...
        case WM_CREATE:
             if(LoadSetting(&setting)==-1)return -1;
             sound=FALSE;
             hWnd=hwnd;
             srand(time(NULL));
			 ...
			 for(int i=0;i<=ITEMKINDS;i++){
					hbititem[i]=(HBITMAP)LoadImage(g_hinst,setting.itemBitNames[i],IMAGE_BITMAP,10,10,LR_LOADFROMFILE);//=LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITITEM1+i));
			}
			 for(int i=0;i<BULLETLEVELS;i++){
					hbitbullet[i]=(HBITMAP)LoadImage(g_hinst,setting.bulletBitNames[i],IMAGE_BITMAP,10,20,LR_LOADFROMFILE);//=LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITBULLET1+i));
			}
             //__asm("int3"); ==DebugBreak();
             hbitenemy=(HBITMAP)LoadImage(g_hinst,setting.enemyBitName/*"res\\Img\\enemy.bmp"*/,IMAGE_BITMAP,20,20,LR_LOADFROMFILE);//hbitenemy=LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITENEMY));
             ///hbitbullet=LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITBULLET));
//             hbitnuclear=(HBITMAP)LoadImage(g_hinst,setting.nuclearBitName,IMAGE_BITMAP,20,20,LR_LOADFROMFILE);//hbitnuclear=LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITNUCLEAR));
             hbitplayer=(HBITMAP)LoadImage(g_hinst,setting.playerBitName,IMAGE_BITMAP,20,20,LR_LOADFROMFILE);//=LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITPLAYER));
             hbitexplosion=(HBITMAP)LoadImage(g_hinst,setting.explosionBitName,IMAGE_BITMAP,20,20,LR_LOADFROMFILE);//LoadBitmap(g_hinst,MAKEINTRESOURCE(IDB_BITEXPLOSION));
             if(hButton==NULL){
        ...
        case WM_KEYDOWN:
             ...
             if(wParam>'0'&&wParam<'9'){
                                        if(sound){
                                                  MessageBox(hWnd,setting.musicNames[wParam-'1'],TEXT("Check"),MB_YESNO);
                                                  myPlaySound(setting.musicNames[wParam-'1']);
                                        }else{
                                              StopSound();
                                        }
             }
