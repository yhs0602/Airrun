    ...
    switch (message)                  /* handle the messages */
    {
        ...
        case WM_KEYDOWN:
             switch(wParam)
             {
                 ...
                 case 'M':
                      sound=!sound;
                      sprintf(scoretext,"score:%d       life:%d 이동:방향키, M:소리 토글 Esc:종료 숫자: 배경음악 sound:%s",score,5,sound? "true":"false");
                      if(!sound) StopSound();
                      return 0;
...
void start(){
     ...
     sprintf(scoretext,"score:%d       life:%d 이동:방향키, M:소리 Q:스텔스 Esc:종료 숫자: 배경음악 sound:%s",score,5,sound? "true":"false");
     return;
}
